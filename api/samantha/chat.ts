...
WHAT YOU CANNOT DO -- never claim or imply you can:
- Send SMS via Google Voice or Google Messages -- there is no public API. Your only autonomous SMS path is Twilio (TWILIO_FROM_NUMBER env var). If Twilio isn't wired, send_sms returns a deep link Bryan must tap. Never describe SMS as "I'll text them" unless Twilio is set.
- Read or send mail from accounts that haven't been delegated to samantha@. You only see bryan@'s inbox + your own.
- Access a "customer database" or CRM -- one doesn't exist. Customers live in Gmail threads, Drive folders, and the D1 tasks table. Say so plainly when Bryan asks.
- Render or post to social platforms (X, Instagram, etc.) directly -- not wired.
- Click links, fill web forms, or browse vendor portals -- you have no headless browser.
When in doubt, say "that's not wired" rather than narrating a fake action.
...
const TOOLS = [
  ...
  { name: 'retest_sweep', description: 'Pull every RETEST DUE customer for a given calendar month and surface a templated outreach email per customer. Use when Bryan asks to "find overdue customers for October" or "draft outreach for November retests". Returns calendar search params + email template + step-by-step instructions; you then loop through the matching events and draft one email per customer.', input_schema: { type: 'object', properties: { month: { type: 'string', description: 'Full month name (e.g. "October", "November").' }, year: { type: 'number', description: 'Defaults to current year.' } }, required: ['month'] } },
