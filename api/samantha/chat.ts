...
const TOOLS = [
  ...
  { name: 'archive_email', description: 'Archive a single email by Gmail message ID. Removes the INBOX label so it disappears from the inbox view but is still searchable in All Mail. Use after read_email when Bryan says to archive, file, or get rid of one specific message.', input_schema: { type: 'object', properties: { message_id: { type: 'string', description: 'Gmail message ID returned by search_emails or read_email.' } }, required: ['message_id'] } },
  { name: 'archive_emails', description: 'Archive up to 100 emails at once by their Gmail message IDs. Use this for bulk inbox cleanup -- e.g. after search_emails returns a list of newsletters or stale threads Bryan wants gone. Removes INBOX label only; messages stay searchable in All Mail (non-destructive).', input_schema: { type: 'object', properties: { message_ids: { type: 'array', items: { type: 'string' }, description: 'Array of Gmail message IDs to archive (max 100 per call).' } }, required: ['message_ids'] } },
